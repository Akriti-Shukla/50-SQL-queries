-- creating order and returns table
-- entering values into it
-- then performing 50+ queries to explore these data
...
select *
from orders_data
WHERE region='Central' AND category='Technology' OR quantity>5;
...
select order_id, order_date, sales, city
from orders_data
WHERE city IN ('Los angeles', 'Henderson');

-- pattern queries
select customer_name, order_date, sales, city
from orders_data
...
UPDATE orders_data set city = NULL where order_id in ('CA-2020-152156', 'US-2019-108966');
...
select category, sum(sales) from orders_data
inner join
returns_data on
orders_data.order_id=returns_data.order_id
group by category;

select od.order_id, rd.return_reason 
from orders_data od
inner join
returns_data rd on
od.order_id=rd.order_id;

select COUNT(*) as order_not_returned
from orders_data od
left join
returns_data rd on
od.order_id=rd.order_id
...
-- String functions
select *, length(customer_name) as cust_length, left(customer_name,4) as left_4letters
from orders_data;



select *, right(order_id,6) as order_number
from orders_data;


select *, substring(order_id,4,4) as year_order
from orders_data;


select *, replace(city, 'Los', 'Ben') as updated_city_name
from orders_data;
...
-- calculating and sorting total sales as per quarter of the order year
select sum(sales) as total_sales,
quarter(order_date) as order_quarter
from orders_data
...
-- analyzing difference of days in between of ship and order date
select order_id, order_date, ship_date,
datediff(ship_date,order_date) as lead_days
from orders_data;

-- adding 5 days to the order date
select order_id, order_date, ship_date,
datediff(ship_date,order_date) as lead_days,
date_add(order_date,interval 5 day) as added_5days
from orders_data;
